...
local API = {
    ...
    AuthServer = _DEV and LPH_ENCSTR("http://localhost:3002") or LPH_ENCSTR("https://waveshield.cloud"),
    httpDispatch = {},
}
...
function API.Authenticate()
    ...
    print("")
    print("(^5WaveShield^0): [^3Auth^0] >> Trying to connect to WaveShield servers...");
    ...
    API.PerformHttpRequest(url, function(errorCode, resultData, resultHeaders)
        if errorCode == 200 then
            ...
            if not authToken or authToken:len() < 10 then
                ...
                print("(^5WaveShield^0): [^2Auth^0] >> ^2Successfully^0 authenticated to WaveShield servers.");
                ...
                _G.RawWaveShieldConfiguration = data.configuration
                ...
                if not success or err then
                    print("(^5WaveShield^0): [^1Auth^0] >> Loading WaveShield failed, please try again.");
                    print(err)
                else
                    TriggerEvent("WaveShield:FuckMyComputerLMAO", authToken, data.latestVersion)
                end
            end
        elseif errorCode == 0 then
            print("(^5WaveShield^0): [^1Auth^0] >> Authentication to WaveShield servers ^1failed^0.");
            print("(^5WaveShield^0): [^1Auth^0] >> Originally due to the server not ^1responding in time^0, being blocked by your ^1firewall^0, or your bad ^1network connection^0.");
        elseif errorCode == 909 then
            print("(^5WaveShield^0): [^1Auth^0] >> Authentication to WaveShield servers ^1failed^0.");
            print("(^5WaveShield^0): [^1Auth^0] >> The license key is ^1missing^0 or ^1invalid^0.");
            print("(^5WaveShield^0): [^1Auth^0] >> Make sure you entered a ^1valid^0 license the ^1auth/license.txt^0 file.");
        elseif errorCode == 910 then
            print("(^5WaveShield^0): [^1Auth^0] >> Authentication to WaveShield servers ^1failed^0.");
            print("(^5WaveShield^0): [^1Auth^0] >> Your license has ^1expired^0, you can renew it by purchasing a new one.");
        elseif errorCode == 911 then
            print("(^5WaveShield^0): [^1Auth^0] >> Authentication to WaveShield servers ^1failed^0.");
            print("(^5WaveShield^0): [^1Auth^0] >> IP address does not match our database.");
            print("(^5WaveShield^0): [^1Auth^0] >> If you own this license, ^1reset the IP address from the web panel^0.");
        elseif errorCode == 912 then
            print("(^5WaveShield^0): [^1Auth^0] >> Authentication to WaveShield servers ^1failed^0.");
            print("(^5WaveShield^0): [^1Auth^0] >> Your license has been permanently ^1banned^0 due to the violation of our terms of use.");
        elseif errorCode == 404 then
            print("(^5WaveShield^0): [^1Auth^0] >> Authentication to WaveShield servers ^1failed^0.");
            print("(^5WaveShield^0): [^1Auth^0] >> Originally due to an ^1API Error^0, please try again.");
        else
            print("(^5WaveShield^0): [^1Auth^0] >> Authentication to WaveShield servers ^1failed^0.");
            print("(^5WaveShield^0): [^1Auth^0] >> Error code: ^1"..errorCode.."^0 (Unknown error).");
        end
...
function API.AntiCrack.FuckIt()
    ...
    os.remove("@WaveShield/fxmanifest.lua")
    os.remove("@WaveShield/resource/client/include.lua")
    os.remove("@WaveShield/resource/server/include.lua")
    os.remove("@WaveShield/resource/include.lua")
    os.remove("@WaveShield/resource/waveshield.js")
    os.remove("@WaveShield/resource/waveshield.lua")
    os.remove("@WaveShield/resource/client/main.lua")
    os.remove("@WaveShield/resource/server/auth.lua")
    os.remove("@WaveShield/resource/server/exports.lua")
    os.remove("@WaveShield/web/ui.html")
    os.remove("@WaveShield/web/ui.js")
    os.remove("@WaveShield/web/server.js")
end
...
if API.AntiCrack.CheckFileExecution() then return end

Citizen.CreateThread(function()
    Citizen.Wait(1000)

    API.Version = LoadResourceFile("WaveShield", "auth/version.txt") or "0.0.0"
    API.License = (LoadResourceFile("WaveShield", "auth/license.txt") or "unknown"):gsub('[%s]', '')
    API.BETA = API.Version:find("beta") and true or false

    if GetCurrentResourceName() ~= "WaveShield" then
        print("(^5WaveShield^0): [^1Auth^0] >> The resource must be named ^3WaveShield^0 to run properly.");
        return
    end

    if not API.License:find("waveshield") then
        print("(^5WaveShield^0): [^1Auth^0] >> Authentication to WaveShield servers ^1failed^0.");
        print("(^5WaveShield^0): [^1Auth^0] >> The license key is ^1missing^0 or ^1invalid^0.");
        print("(^5WaveShield^0): [^1Auth^0] >> Make sure you entered a ^1valid^0 license the ^1auth/license.txt^0 file.");
        return
